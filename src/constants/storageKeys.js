// src/constants/storageKeys.js
// Single source of truth for all AsyncStorage keys used across the app.

export const DISPLAY_NAME_KEY  = '@peerreach_display_name';
export const SEND_BATTERY_KEY  = '@peerreach_send_battery';
export const WEATHER_CACHE_KEY = '@peerreach_weather_v2';
export const THEME_PREF_KEY    = '@peerreach_theme';
export const LAST_LOCATION_KEY  = '@peerreach_last_location';
export const SHOW_SOS_FINDME_KEY = '@peerreach_show_sos_findme';
export const BRIDGEFY_EVER_INIT_KEY = '@peerreach_bridgefy_ever_init';
